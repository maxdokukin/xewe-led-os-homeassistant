 ...
 * -----------------------------------------
 * On/off LED integrated with Home Assistant via MQTT auto-discovery.
 *
 ...
 *      RETAINED light discovery config, and drive LED_PIN.
 *
...
// ---- MQTT -------------------------------------------------------------------
// Publish the current light state. Call this on EVERY state change, not just on
// MQTT commands — a physical button / HomeKit / Alexa change must also be
// published here, otherwise Home Assistant won't see it (that push is the whole
// reason this firmware uses MQTT instead of HTTP polling).
void publishState() {
  mqtt.publish(stateTopic.c_str(), lightOn ? "ON" : "OFF", true);
}
...
void onMessage(char *topic, byte *payload, unsigned int length) {
  ...
  if (msg == "ON") applyLight(true);
  else if (msg == "OFF") applyLight(false);
  publishState();
}
